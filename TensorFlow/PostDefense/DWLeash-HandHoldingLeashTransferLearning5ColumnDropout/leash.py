...
steps = 5                           # number of columns for steps in paper
transition_dist = 1
...
def columnActivation(aug_x, column_num, fwd_dict, keep_prob):
...
def doForwardPass(x, out_locs, gt_loc, keep_prob):
  ...
  offset_grid_list = []
  for xi in range(grid_x.shape[0]):
    for yi in range(grid_y.shape[0]):
  ...
  for i in range(steps):
    res_step = columnActivation(aug_x, i, fwd_dict, keep_prob)
...
def train(res_aux, global_step):
  ...
  ret_dict['w_act'] = res_aux['w_act_4']    #DELETE     
  ret_dict['nw_act'] = res_aux['nw_act_4']    #DELETE       
  ...
  var_list = var_list + tf.get_collection('col3row1weights')
  var_list = var_list + tf.get_collection('col3row1biases')
  var_list = var_list + tf.get_collection('col4row1weights')
  var_list = var_list + tf.get_collection('col4row1biases')    
  var_list = var_list + getSharedParametersList()  

  total_loss = tf.reduce_sum(res_aux['loss'])
  # a_optimizer = tf.train.GradientDescentOptimizer(0.001, name='GradientDescent')

  # a_optimizer = tf.train.MomentumOptimizer(learning_rate=0.001, momentum= 0.9, name='MomentumOptimizer')
