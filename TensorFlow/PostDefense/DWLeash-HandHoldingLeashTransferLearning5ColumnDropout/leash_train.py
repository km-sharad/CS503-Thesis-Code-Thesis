...
total_visible_training_images = 310     # Number of training images where car door handle is visible
...
def getValidationImageMetaRecords():
  all_validation_visible_idx = [x for x in range(0, total_visible_validation_images)]
...
res_aux = leash.doForwardPass(vgg.pool4,out_locs,org_gt_coords,keep_prob)
...
# with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:  
  ...
  for epoch in range(max_steps):
    ...
    for batch in range(int(len(anno_file_batch_rows)/batch_size)):
      ...
      # print('**** w_act: ', out_dict['w_act'][3,2:4,4:8,:])       #DELETE  
      # print('**** nw_act: ', out_dict['nw_act'][3,2:4,4:8,:])       #DELETE        
      # print('**** w_act: ', out_dict['w_act'].shape)       #DELETE  
      # print('**** nw_act: ', out_dict['nw_act'].shape)       #DELETE        

      # out_f = open('act_file.txt', 'a+')
      # out_f.write('*** w_act: ' + ' ' + str(out_dict['w_act']))
      # out_f.write('*** nw_act: ' + ' ' + str(out_dict['nw_act']))
      # out_f.close()      
      ...
      if(math.isnan(out_dict['loss'])):
        #The loss sometimes becomes 0 after we added dropout for training.
        #Reload the model and resume training
        saver.restore(sess, "./ckpt/model" + str(int(epoch-2)) + ".ckpt")

        out_f = open('err_file.txt', 'a+')
        out_f.write(str(epoch) + ' ' + str(batch) + ' ' + str(out_dict['loss']) + '\n')
        out_f.write('NaN loss after epoch: ' + str(epoch) + ' batch: ' + str(batch) + '\n')
        out_f.write('Restarted Training from epoch: ' + str(int(epoch)))
        out_f.close()
