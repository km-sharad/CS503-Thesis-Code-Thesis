...
def distorted_inputs(stats_dict, batch_size, anno_file_batch_rows):
  ...
  x = np.arange(start_offset, (padded_images.shape[1]+1)*2, output_stride)
  y = np.arange(start_offset, (padded_images.shape[2]+1)*2, output_stride)
...
def getImage(meta_rec, stats_dict):
    im_meta_dict = {}
    im_meta_dict['gt_x_coord'] = int(float(meta_rec[0]))    #holding leash x
    im_meta_dict['gt_y_coord'] = int(float(meta_rec[1]))    #holding leash y
    im_meta_dict['img_size'] = [int(i) for i in meta_rec[5].split(',')]
    im_meta_dict['bbox'] = [int(i) for i in meta_rec[6].strip()[0:len(meta_rec[6]) - 1].split(',')]    

    image = Image.open(data_dir + meta_rec[4])
    # image = skimage.io.imread(data_dir + meta_rec[4])

    im = np.array(image)
    if(len(im.shape) == 2):
      #monochrome image, add the third channel
      im = np.stack((image,)*3)
      print('monochrome image: ', meta_rec[4])
    ...
    try:
      ...
      print('padding error for image: ', meta_rec[4])
