...
def getValidationImageMetaRecords():
  ...
  validation_anno_file = open('cdhd_anno_validation_data.txt')
...
def calculteNormalizedValidationDistance(pred, original, bbox_heights):
  ...
  for i in range(original.shape[0]): 
    ...
    total_normalized_distance = total_normalized_distance + ',' + str(normalized_dist)
...
# with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:  
  ...
  #saver.restore(sess, "./ckpt/model3867.ckpt")
